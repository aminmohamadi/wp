...
use SiliconElementor\Modules\Posts\Traits\Button_Widget_Trait;
...
abstract class Posts_Base extends Base_Widget {
	...
	public function register_load_more_message_style_controls() {
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'No More Posts Message', 'silicon-elementor' ),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
				'condition' => [
					'pagination_type' => 'load_more_on_click',
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Spinner Color', 'silicon-elementor' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '',
				'selectors' => [
	...
	public function register_pagination_section_controls() {
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Page Limit', 'silicon-elementor' ),
				'default'   => '5',
				'condition' => [
					'pagination_type!' => [
						'load_more_on_click',
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Shorten', 'silicon-elementor' ),
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Previous Label', 'silicon-elementor' ),
				'default'   => esc_html__( '&laquo; Previous', 'silicon-elementor' ),
				'condition' => [
					'pagination_type' => [
						'prev_next',
						'numbers_and_prev_next',
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Next Label', 'silicon-elementor' ),
				'default'   => esc_html__( 'Next &raquo;', 'silicon-elementor' ),
				'condition' => [
					'pagination_type' => [
						'prev_next',
						'numbers_and_prev_next',
		...
		$this->register_button_content_controls(
			[
				'button_text'            => esc_html__( 'Load More', 'silicon-elementor' ),
				'control_label_name'     => esc_html__( 'Button Text', 'silicon-elementor' ),
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Custom Messages', 'silicon-elementor' ),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => '',
				'condition' => [
					'pagination_type' => [
		...
		$this->add_control(
			...
			[
				...
				'default'     => esc_html__( 'No more posts to show', 'silicon-elementor' ),
				'condition'   => [
					'pagination_type'                     => [
						'load_more_on_click',
						'load_more_infinite_scroll',
		...
		$this->add_control(
			'navigation_classes',
			[
				'label' => esc_html__( 'Navigation Class', 'silicon-elementor' ),
				'type'  => Controls_Manager::TEXT,
				'title' => esc_html__( 'Additional CSS class that you want to apply to the ul.pagination tag', 'silicon-elementor' ),
			]
		);
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Colors', 'silicon-elementor' ),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
	...
	public function get_wp_link_page( $i ) {
		...
		if ( is_preview() ) {
			if ( ( 'draft' !== $post->post_status ) && isset( $_GET['preview_id'], $_GET['preview_nonce'] ) ) {
				$query_args['preview_id']    = silicon_elementor_clean( wp_unslash( $_GET['preview_id'] ) );
				$query_args['preview_nonce'] = silicon_elementor_clean( wp_unslash( $_GET['preview_nonce'] ) );
			}
	...
	protected function get_pagination_type_options() {
		return [
			...
			'numbers'                       => esc_html__( 'Numbers', 'silicon-elementor' ),
			'prev_next'                     => esc_html__( 'Previous/Next', 'silicon-elementor' ),
			'numbers_and_prev_next'         => esc_html__( 'Numbers', 'silicon-elementor' ) . ' + ' . esc_html__( 'Previous/Next', 'silicon-elementor' ),
			self::LOAD_MORE_ON_CLICK        => esc_html__( 'Load on Click', 'silicon-elementor' ),
			self::LOAD_MORE_INFINITE_SCROLL => esc_html__( 'Infinite Scroll', 'silicon-elementor' ),
		];
	}
