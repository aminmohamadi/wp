<?php
namespace SiliconElementor\Modules\QueryControl;

use Elementor\Controls_Manager;
use Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use Elementor\Widget_Base;
use SiliconElementor\Base\Module_Base;
use SiliconElementor\Modules\QueryControl\Controls\Group_Control_Posts;
use SiliconElementor\Modules\QueryControl\Controls\Group_Control_Query;
use SiliconElementor\Modules\QueryControl\Controls\Group_Control_Related;
use SiliconElementor\Modules\QueryControl\Classes\Elementor_Post_Query;
use SiliconElementor\Modules\QueryControl\Classes\Elementor_Related_Query;
use SiliconElementor\Modules\QueryControl\Controls\Query;
use SiliconElementor;
...
class Module extends Module_Base {
	...
	public static function add_exclude_controls( $widget ) {
		...
		$widget->add_control(
			'exclude',
			[
				'label'       => __( 'Exclude', 'silicon-elementor' ),
				'type'        => Controls_Manager::SELECT2,
				'multiple'    => true,
				'options'     => [
					'current_post'     => __( 'Current Post', 'silicon-elementor' ),
					'manual_selection' => __( 'Manual Selection', 'silicon-elementor' ),
				],
				'label_block' => true,
			]
		);
		...
		$widget->add_control(
			'exclude_ids',
			[
				'label'        => __( 'Search & Select', 'silicon-elementor' ),
		...
		$widget->add_control(
			'avoid_duplicates',
			[
				'label'       => __( 'Avoid Duplicates', 'silicon-elementor' ),
				'type'        => Controls_Manager::SWITCHER,
				'default'     => '',
				'description' => __( 'Set to Yes to avoid duplicate posts from showing up on the page. This only affects the frontend.', 'silicon-elementor' ),
			]
		);
	...
	public function ajax_posts_filter_autocomplete_deprecated( $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters_deprecated( 'silicon_elementor/query_control/get_autocomplete/' . $data['filter_type'], $data, '3.0.0', 'elementor/query/get_autocomplete/' . $data['filter_type'] );
	...
	public function ajax_posts_filter_autocomplete( array $data ) {
		...
		switch ( $query_data['object'] ) {
			...
			case self::QUERY_OBJECT_LIBRARY_TEMPLATE:
				...
				foreach ( $query->posts as $post ) {
					$document = SiliconElementor\Plugin::elementor()->documents->get( $post->ID );
					if ( $document ) {
	...
	public function ajax_posts_control_value_titles_deprecated( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters_deprecated( 'silicon_elementor/query_control/get_value_titles/' . $request['filter_type'], $request, '3.0.0', 'elementor/query/get_value_titles/' . $request['filter_type'] );
	...
	public function register_controls() {
		$controls_manager = SiliconElementor\Plugin::elementor()->controls_manager;
	 ...
	 * Get Query.
	 *
	 * @param \SiliconElementor\Base\Base_Widget $widget widget.
	...
	protected function add_actions() {
		...
		add_filter( 'silicon_elementor/editor/localize_settings', [ $this, 'localize_settings' ] );
