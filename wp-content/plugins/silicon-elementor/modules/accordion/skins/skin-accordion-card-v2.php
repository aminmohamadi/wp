 ...
 */
class Skin_Accordion_Card_V2 extends Skin_Base {
	...
	public function get_title() {
		return esc_html__( 'Card V2', 'silicon-elementor' );
	}
	...
	public function remove_content_controls() {
		...
		foreach ( $disable_controls as $control ) {
			$this->parent->update_control(
				...
				[
					'condition' => [
						'_skin!' => [ 'sn-accordion-card', 'sn-accordion-silicon', 'sn-accordion-card-2' ],
					],
				]
			);
		}
	...
	public function modifying_style_sections( Widget_Base $widget ) {
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .silicon-accordion .silicon-accordion__contents' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .silicon-accordion .silicon-accordion__contents' => 'color: {{VALUE}} !important;',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .silicon-accordion .silicon-accordion__contents',
				'global'   => [
		...
		$this->add_control(
			'content_wrap_css',
			[
				'label'       => esc_html__( 'Wrap CSS', 'silicon-elementor' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => 'bg-white border-0 rounded-3 shadow-sm mb-3',
	...
	public function render() {
		...
		$widget->add_render_attribute( 'accordion_content', 'class', [ 'silicon-accordion-body', 'accordion-body', 'silicon-accordion__contents', 'fs-sm', 'pt-0' ] );

		if ( ! empty( $content_css ) ) {
			$widget->add_render_attribute( 'content', 'class', $content_css );
		}

		$content_wrap_css = $this->get_instance_value( 'content_wrap_css' );

		if ( ! empty( $content_wrap_css ) ) {
			$widget->add_render_attribute( 'accordion_wrap_css', 'class', [ 'accordion-item ', $content_wrap_css ] );
		}
		...
		<div class="silicon-accordion" id="accordion-<?php echo esc_attr( $id_int ); ?>">
			...
			foreach ( $settings['tabs'] as $index => $item ) :
				...
				?>
			<div <?php $widget->print_render_attribute_string( 'accordion_wrap_css' ); ?>>
