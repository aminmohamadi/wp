...
if ( ! function_exists( 'silicon_get_field' ) ) {
	/**
	 * Query Silicon get field.
	 *
	 * @param array $field  post ID.
	 * @param bool  $post_id  post ID.
	 * @param bool  $format_value  post ID.
	 */
	function silicon_get_field( $field, $post_id = false, $format_value = true ) {
...
if ( ! function_exists( 'silicon_is_mas_static_content_activated' ) ) {
	/**
	 * Query MAS Static Content activation
	 */
	function silicon_is_mas_static_content_activated() {
		return class_exists( 'Mas_Static_Content' ) ? true : false;
	}
}

if ( ! function_exists( 'silicon_is_acf_activated' ) ) {
	/**
	 * Query ACF activation.
	 */
	function silicon_is_acf_activated() {
		return function_exists( 'get_field' ) ? true : false;
	}
}

if ( ! function_exists( 'silicon_is_ocdi_activated' ) ) {
	/**
	 * Check if One Click Demo Import is activated
	 */
	function silicon_is_ocdi_activated() {
...
require_once get_template_directory() . '/inc/functions/template.php';
 ...
 */
function silicon_do_shortcode( $tag, array $atts = array(), $content = null ) {
 ...
 * Accounts for the Silicon Designer and Silicon Powerpack content background option.
 *
 * @since  1.6.0
 * @return string the background color
 */
function silicon_get_content_background_color() {
	if ( class_exists( 'Silicon_Designer' ) ) {
		$content_bg_color = get_theme_mod( 'sd_content_background_color' );
		$content_frame    = get_theme_mod( 'sd_fixed_width' );
	}

	if ( class_exists( 'Silicon_Powerpack' ) ) {
	...
	if ( class_exists( 'Silicon_Powerpack' ) || class_exists( 'Silicon_Designer' ) ) {
...
/**
 * Apply inline style to the Silicon header.
 *
 * @uses  get_header_image()
 * @since  2.0.0
 */
function silicon_header_styles() {
	...
	$styles = apply_filters( 'silicon_header_styles', $styles );
...
/**
 * Apply inline style to the Silicon homepage content.
 *
 * @uses  get_the_post_thumbnail_url()
 * @since  2.2.0
 */
function silicon_homepage_content_styles() {
	...
	$styles = apply_filters( 'silicon_homepage_content_styles', $styles );
 ...
 * @since  1.0.0
 */
function silicon_adjust_color_brightness( $hex, $steps, $opacity = 1 ) {
 ...
 * @since  1.3.0
 */
function silicon_sanitize_choices( $input, $setting ) {
 ...
 * @since  1.5.0
 */
function silicon_sanitize_checkbox( $checked ) {
	return ( ( isset( $checked ) && true === $checked ) ? true : false );
}

if ( ! function_exists( 'silicon_option_enabled_post_types' ) ) {
	 ...
	 * @return array
	 */
	function silicon_option_enabled_post_types() {
		$post_types = [ 'post', 'page', 'jetpack-portfolio' ];
		return apply_filters( 'silicon_option_enabled_post_types', $post_types );
	}
}

if ( ! function_exists( 'silicon_default_colors' ) ) {
	/**
	 * Default theme colors of Silicon.
	 *
	 * @return array
	 */
	function silicon_default_colors() {
		$colors = array(
			array(
				'_id'   => 'primary',
				'title' => esc_html__( 'Primary', 'silicon' ),
				'color' => '#6366f1',
			),
			array(
				'_id'   => 'secondary',
			...
			array(
				'_id'   => 'success',
				'title' => esc_html__( 'Success', 'silicon' ),
				'color' => '#22c55e',
			),
			array(
				'_id'   => 'info',
				'title' => esc_html__( 'Info', 'silicon' ),
				'color' => '#4c82f7',
			),
			array(
				'_id'   => 'warning',
				'title' => esc_html__( 'Warning', 'silicon' ),
				'color' => '#ffba08',
			),
			array(
				'_id'   => 'danger',
				'title' => esc_html__( 'Danger', 'silicon' ),
				'color' => '#ef4444',
			),
			array(
				'_id'   => 'light',
				'title' => esc_html__( 'Light', 'silicon' ),
				'color' => '#ffffff',
			),
			array(
				'_id'   => 'dark',
				'title' => esc_html__( 'Dark', 'silicon' ),
				'color' => '#131022',
			),
		);
		return apply_filters( 'silicon_default_colors', $colors );
	}
}
